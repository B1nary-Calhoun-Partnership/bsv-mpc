...
//! (MIT licensed, © Calhooon Contributors). Byte-identical to
//! `~/bsv/rust-message-box/src/engineio/codec.rs`. Source upstream:
//! <https://github.com/Calhooon/bsv-messagebox-cloudflare>.
//!
//! Direction-agnostic — encodes + decodes both server-bound AND
//! client-bound packets. Phase H POC consumes it on the client side
//! (audit `docs/PHASE-H-AUDIT.md` §11.2 revised, Plan A1).
//!
//! Ecosystem follow-up tracked in `STATUS.md`: extract into a shared
//! `bsv-engineio-rs` crate so both servers + this client depend on
//! one copy instead of duplicating.
//!
//! ## Engine.IO packets
//!
...
//! by the AuthSocket transport, but the parser handles arbitrary
//! namespaces correctly so non-standard clients still parse.
...
/// Decoded Socket.IO packet. We don't bother with `BINARY_EVENT` /
/// `BINARY_ACK` because the AuthSocket transport never sends binary
/// attachments — the JSON payload carries everything.
#[derive(Debug, Clone, PartialEq)]
pub enum SocketIoPacket {
    /// CONNECT (type 0). Client → server: optional auth payload.
    /// Server → client: `{ sid: <socket-sid> }`.
    Connect { nsp: String, data: Option<Value> },
    ...
    ConnectError { nsp: String, data: Option<Value> },
}
