...
async fn ceremony() {
    ...
    let bundle_dir =
        std::env::temp_dir().join(format!("bsvmpc-98-repro-{}", std::process::id()));
    ...
    eprintln!("[#98 repro] ✔ connected; running the n-party presign-over-relay (the #98 hot spot)…");
    ...
    eprintln!("[#98 repro] ✔✔ presign + sign COMPLETED on a single-threaded FFI-emulating executor");
}
