...
async fn full_reshare_2of2_to_2of3_over_messagebox() {
    ...
    for i in 0..n_new as usize {
        dkg_listeners.push(
            MessageBoxListener::start(clients[i].clone(), BOX_DKG, dkg_handlers[i].handler_fn())
                .await
                .expect("dkg listener"),
        );
    }
    ...
    for j in 0..n_new {
        let peers: Vec<(u16, String)> = (0..n_new)
            ...
            .collect();
        let (rx, out) = dkg_handlers[j as usize]
    ...
    for j in 0..n_new as usize {
        let r = tokio::time::timeout(timeout, dkg_rxs.remove(0))
            .await
            .unwrap_or_else(|_| panic!("agent {j} DKG timeout"))
            .expect("dkg channel");
        throwaway.push(r);
    }
    ...
    for i in 0..n_new as usize {
        pss_listeners.push(
            MessageBoxListener::start(
                clients[i].clone(),
                BOX_REFRESH,
                reshar_handlers[i].handler_fn(),
            )
            .await
            .expect("pss listener"),
        );
    ...
    for j in 0..n_new {
        ...
        let (rx, out) = reshar_handlers[j as usize]
    ...
    for (j, tw) in throwaway.iter().enumerate() {
        let pss = tokio::time::timeout(timeout, pss_rxs.remove(0))
            .await
            .unwrap_or_else(|_| panic!("agent {j} PSS timeout"))
            .expect("pss channel");
        assert_eq!(
            pss.joint_pubkey_compressed, jpk_bytes,
            "agent {j}: joint pubkey unchanged"
        );
        let combined_json =
            combine_reshared_with_aux(&pss.incomplete_share_json, &tw.share.ciphertext)
    ...
    eprintln!(
        "✔ all 3 agents combined: PSS reshare (K) + throwaway-DKG aux → new 2-of-3 KeyShares"
    );
