...
fn blum(rng: &mut impl rand::RngCore, bits: u32) -> cggmp24::backend::Integer {
    ...
    loop { let n = Integer::generate_prime(rng, bits); if n.mod_u(4) == 3 { break n; } }
}
fn primes(rng: &mut impl rand::RngCore) -> cggmp24::PregeneratedPrimes<SecurityLevel128> {
    ...
    cggmp24::PregeneratedPrimes::try_from([blum(rng,b),blum(rng,b),blum(rng,b),blum(rng,b)]).expect("primes")
}
...
async fn reshar_3of4_to_4of6_over_messagebox() {
    ...
    let subset_old_eval: Vec<NonZero<Scalar<Secp256k1>>> = (0..new_t).map(|k| old_eval[k as usize]).collect();

    // ── 6 party-agents on the live relay ──
    let session_id = SessionId::from_str_hash(&format!("reshar-e2e-{}", {
        let mut b = [0u8; 8]; rand::rngs::OsRng.fill_bytes(&mut b); hex::encode(b)
    }));
    ...
    for _ in 0..n_new {
        ...
        privs.push(p); clients.push(c); pubs.push(pubhex);
    }
    ...
    for j in 0..n_new as usize {
        let l = MessageBoxListener::start(clients[j].clone(), BOX_REFRESH, handlers[j].handler_fn())
    ...
    for j in 0..n_new {
        ...
        let (rx, out) = handlers[j as usize].initiate(config, peers).await.expect("initiate");
        rxs.push(rx);
        all_round1.push((j, out));
    }
    for (j, out) in all_round1 {
        for o in out {
            clients[j as usize].send_round_message(&o.recipient_pub_hex, &o.message_box, &o.round_msg, o.params)
                .await.expect("ship round-1");
        }
    }
    ...
    for h in &handlers { assert_eq!(h.live_session_count(), 0, "cleaned up"); }
    for l in listeners { let _ = tokio::time::timeout(Duration::from_secs(10), l.shutdown()).await; }
