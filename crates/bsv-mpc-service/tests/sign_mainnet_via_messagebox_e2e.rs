...
/// **M1 (MPC-Spec #12) gate** — a real mainnet TX signed by a **2-of-3** quorum
/// over MessageBox. Three bsv-mpc cosigners run a real 2-of-3 DKG (one stands in
/// for Binary's M1 cosigner — it runs bsv-mpc), then **two of the three** sign a
/// real BSV mainnet sighash via the relay and broadcast it. No shortcuts: real
/// safe primes, real relay, real sats, real ARC broadcast.
#[tokio::test]
async fn within_stack_2of3_sign_mainnet_tx_via_messagebox() {
    let Some(relay_url) = opt_in() else {
        eprintln!(
            "MESSAGEBOX_RELAY_URL + E2E_MAINNET=1 not both set — skipping M1 2-of-3 mainnet TX.
To run (BURNS REAL SATS):
  MESSAGEBOX_RELAY_URL=https://rust-message-box.dev-a3e.workers.dev \\
  E2E_MAINNET=1 \\
    cargo test -p bsv-mpc-service --test sign_mainnet_via_messagebox_e2e \\
      --release within_stack_2of3 -- --nocapture --test-threads=1"
        );
        return;
    };
    let _ = tracing_subscriber::fmt::try_init();
    ...
    // ============ 2-of-3 DKG (3 parties) ============
    let config = ThresholdConfig::new(2, 3).expect("2-of-3");
    let signers = vec![0u16, 1u16]; // the quorum that will SIGN (any 2 of 3)

    // All three participate in DKG; the signing handler quorum is the 2 signers.
    let alice = boot_cosigner(&relay_url, 0, config, signers.clone()).await;
    let bob = boot_cosigner(&relay_url, 1, config, signers.clone()).await;
    let carol = boot_cosigner(&relay_url, 2, config, signers.clone()).await;
    eprintln!("✔ alice = {} (signer)", alice.pub_hex);
    eprintln!("✔ bob   = {} (signer)", bob.pub_hex);
    eprintln!(
        "✔ carol = {} (DKG only — Binary M1 stand-in)",
        carol.pub_hex
    );

    eprintln!("(generating 3 Paillier safe-prime sets — ~60-90s each, parallel)");
    let primes_t0 = std::time::Instant::now();
    ...
    .await
    .unwrap();
    eprintln!("✔ 3 prime sets in {:?}", primes_t0.elapsed());
    ...
    alice
        .dkg_handler
        .seed_primes_for(dkg_session_id, primes.remove(0));
    bob.dkg_handler
        .seed_primes_for(dkg_session_id, primes.remove(0));
    carol
        .dkg_handler
    ...
    // Initiate ALL THREE before shipping any round-1 (so every coordinator
    // exists — otherwise an early message is dropped as "unknown session").
    let (a_rx, a_out) = alice
        .dkg_handler
        .initiate(
            dkg_session_id,
            vec![(1, bob.pub_hex.clone()), (2, carol.pub_hex.clone())],
        )
        .await
        .expect("alice dkg initiate");
    let (b_rx, b_out) = bob
        ...
        .initiate(
            dkg_session_id,
            vec![(0, alice.pub_hex.clone()), (2, carol.pub_hex.clone())],
        )
        .await
        .expect("bob dkg initiate");
    let (c_rx, c_out) = carol
        .dkg_handler
        .initiate(
            dkg_session_id,
            vec![(0, alice.pub_hex.clone()), (1, bob.pub_hex.clone())],
        )
        .await
        .expect("carol dkg initiate");

    for (cosigner, outs) in [(&alice, a_out), (&bob, b_out), (&carol, c_out)] {
        for out in outs {
            cosigner
                .client
                .send_round_message(
                ...
                .await
                .expect("dkg round-1 send");
        }
    }

    let dkg_t0 = std::time::Instant::now();
    let (a_dkg, b_dkg, c_dkg) = tokio::join!(
        tokio::time::timeout(Duration::from_secs(300), a_rx),
        tokio::time::timeout(Duration::from_secs(300), b_rx),
        tokio::time::timeout(Duration::from_secs(300), c_rx),
    );
    let a_dkg = a_dkg.unwrap().unwrap();
    let b_dkg = b_dkg.unwrap().unwrap();
    let c_dkg = c_dkg.unwrap().unwrap();
    assert_eq!(
        a_dkg.joint_key.compressed, b_dkg.joint_key.compressed,
        "alice/bob joint pubkey MUST agree"
    );
    assert_eq!(
        a_dkg.joint_key.compressed, c_dkg.joint_key.compressed,
        "carol joint pubkey MUST agree — proves 2-of-3 DKG"
    );
    let mut joint_pubkey_arr = [0u8; 33];
    joint_pubkey_arr.copy_from_slice(&a_dkg.joint_key.compressed);
    let joint_pubkey =
        PublicKey::from_bytes(&joint_pubkey_arr).expect("joint pubkey from compressed bytes");
    let joint_address = a_dkg.joint_key.address.clone();
    eprintln!(
        "✔ 2-of-3 DKG complete in {:?} — joint_pubkey={} address={}",
        dkg_t0.elapsed(),
        hex::encode(joint_pubkey_arr),
        joint_address
    );
    ...
    // ============ Fund the joint address via wallet:3321 ============
    // Force a synchronous broadcast so the funding tx actually hits the network
    // (the child spend chains off it); the default delayed broadcast can leave it
    // un-sent → the spend would be unspendable.
    let http = reqwest::Client::new();
    let funding_amount: u64 = 1500;
    let joint_locking = p2pkh_locking_script(&joint_pubkey.hash160());
    ...
    let fund_resp = http
        ...
        .header("Content-Type", "application/json")
        .json(&serde_json::json!({
            "description": "bsv-mpc M1 2-of-3 MessageBox-signed mainnet test",
            "outputs": [{
                "satoshis": funding_amount,
                "lockingScript": hex::encode(&joint_locking),
                "outputDescription": "MPC joint P2PKH (2-of-3)"
            }],
            "options": { "acceptDelayedBroadcast": false }
        }))
        .send()
        .await
        .expect("wallet:3321 reachable — start the wallet first");
    let fund_status = fund_resp.status();
    let fund_text = fund_resp.text().await.unwrap_or_default();
    assert!(
        fund_status.is_success(),
    ...
    // ============ Sign with 2 of the 3 (alice + bob) via MessageBox ============
    // Carol holds a share but does NOT sign — this is the 2-of-3 threshold:
    // any 2 of the 3 produce a valid signature under the joint key.
    let sign_session_id = {
        let mut b = [0u8; 32];
        rand::rngs::OsRng.fill_bytes(&mut b);
        SessionId(b)
    };
    let agent_id = a_dkg.session_id.hex();
    ...
    assert_eq!(
        alice_sig.signature, bob_sig.signature,
        "BOTH signers MUST produce the byte-identical DER signature"
    );
    eprintln!(
        "✔ 2-of-3 signing complete in {:?} — DER sig {} bytes (both signers byte-identical)",
        sig_t0.elapsed(),
        alice_sig.signature.len()
    );
    ...
    assert!(
        joint_pubkey.verify(&sighash, &bsv_sig),
        "PRE-FLIGHT: 2-of-3 signature MUST verify against joint pubkey before broadcast"
    );
    eprintln!("✔ pre-flight ECDSA verify against joint pubkey: PASS");

    // ============ Build unlocking script + serialize + broadcast ============
    let tx_sig = TransactionSignature::new(bsv_sig, scope);
    let checksig_bytes = tx_sig.to_checksig_format();
    let compressed_joint_pub = joint_pubkey.to_compressed();
    let unlocking = p2pkh_unlocking_script(&checksig_bytes, &compressed_joint_pub);
    ...
    eprintln!();
    eprintln!("╔═══════════════════════════════════════════════════════════════╗");
    eprintln!("║  M1 2-OF-3 MAINNET TX — SIGNED VIA MESSAGEBOX                ║");
    eprintln!("╚═══════════════════════════════════════════════════════════════╝");
    eprintln!("  signers: alice(0) + bob(1); carol(2) held a share but did NOT sign");
    eprintln!("  joint_pubkey: {}", hex::encode(joint_pubkey_arr));
    eprintln!("  joint_address: {}", joint_address);
    eprintln!("  funding_txid: {}", fund_txid);
    eprintln!("  spending_txid: {}", txid_hex);
