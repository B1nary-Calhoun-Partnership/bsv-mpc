...
async fn phaseA_dkg_with_delayed_party0_over_messagebox() {
    ...
    eprintln!("⏱  party 0 will join phase A ~{:?} late (override via PARTY0_DELAY_SECS)", delay);
    ...
    let dkg_handlers: Vec<DkgHandler> =
        (0..n_new).map(|i| DkgHandler::new(new_cfg, i, fresh_storage())).collect();
    ...
    async fn arm_party(
        ...
        let peers: Vec<(u16, String)> = (0..n_new)
            ...
            .collect();
        let (rx, out) = handler.initiate(dkg_session, peers).await.expect("dkg initiate");
        for o in out {
            client
    ...
    eprintln!("✔ all 3 parties armed + round-1 shipped at +{:?}", t0.elapsed());
    ...
    eprintln!("(generating Paillier primes — parties 1,2 seeded now; party 0 seeded {delay:?} late)");
    let mut primes = tokio::task::spawn_blocking(|| {
        (0..3)
            .map(|_| std::thread::spawn(|| PregeneratedPrimes::<SecurityLevel128>::generate(&mut rand::rngs::OsRng)))
            .collect::<Vec<_>>()
            .into_iter()
            .map(|h| h.join().unwrap())
            .collect::<Vec<_>>()
    ...
    eprintln!("✔ parties 1 & 2 primes seeded at +{:?}; party 0 seeding in {delay:?}", t0.elapsed());
    ...
    eprintln!("✔ awaiting all 3 aux (party 0's primes seeded late at ~+{:?})", delay);
    ...
    for (j, rx) in rxs.drain(..) {
        match tokio::time::timeout(timeout, rx).await {
            ...
            Err(_) => {
                ...
                panic!("party {j} timed out awaiting throwaway DKG aux (delayed-party0 reproduction)");
            }
        }
    }
    for l in [l0, l1, l2] {
    ...
    assert_eq!(completed, 3, "all 3 parties must complete the throwaway DKG");
